        ...
        if (url.includes('/login') || url.includes('/signup') || url.includes('/register')) {
            try {
                ...
                if (token) {
                    ...
                    if (tokenDisplay) {
                        tokenDisplay.textContent = `Logged in as: ${getTokenUser(token)}`;
...
function getTokenUser(token) {
    try {
        ...
        if (payload) {
            return payload.firstName || payload.name || payload.email || payload.sub || 'Unknown User';
        }
    } catch (e) {
...
// Test multipart form data endpoints (for biometric uploads)
async function testFormDataEndpoint(url, method, formData, button) {
...
async function testFileEndpoint(url, method, fileInput, button) {
    ...
    formData.append('file', file); // Generic file field
    
    testFormDataEndpoint(url, method, formData, button);
}
