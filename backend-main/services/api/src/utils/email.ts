...
export class Email {
  ...
  private async send(
    ...
    // Bypass email sending in development if SEND_EMAILS is not set to 'true'
    if (process.env.NODE_ENV !== 'production' && process.env.SEND_EMAILS !== 'true') {
      console.log(`[EMAIL MOCK] Would send email to: ${this.to}, subject: "${subject}", template: "${template}"`);
      return;
    }
  ...
  async checkInReminder() {
    try {
      ...
      // Bypass email sending in development if SEND_EMAILS is not set to 'true'
      if (process.env.NODE_ENV !== 'production' && process.env.SEND_EMAILS !== 'true') {
        console.log(`[EMAIL MOCK] Would send check-in reminder to: ${this.to}`);
        return;
      }
  ...
  async announcementEmail(data: any) {
    try {
      ...
      // Bypass email sending in development if SEND_EMAILS is not set to 'true'
      if (process.env.NODE_ENV !== 'production' && process.env.SEND_EMAILS !== 'true') {
        console.log(`[EMAIL MOCK] Would send announcement email to: ${data.email}`);
        return;
      }
  ...
  async sendReceipt(subject: string, flightInfo: any) {
    try {
      // Bypass email sending in development if SEND_EMAILS is not set to 'true'
      if (process.env.NODE_ENV !== 'production' && process.env.SEND_EMAILS !== 'true') {
        console.log(`[EMAIL MOCK] Would send receipt email to: ${flightInfo.booking.additionalInfo.email}`);
        return;
      }
  ...
  async sendReceipt2(subject: string, flightData: any) {
    try {
      // Bypass email sending in development if SEND_EMAILS is not set to 'true'
      if (process.env.NODE_ENV !== 'production' && process.env.SEND_EMAILS !== 'true') {
        console.log(`[EMAIL MOCK] Would send receipt email to: ${flightData.booking.additionalInfo.email}`);
        return;
      }
  ...
  async sendSupportTicketCreated(ticket: any) {
    try {
      ...
      // Bypass email sending in development if SEND_EMAILS is not set to 'true'
      if (process.env.NODE_ENV !== 'production' && process.env.SEND_EMAILS !== 'true') {
        console.log(`[EMAIL MOCK] Would send support ticket created email to: ${this.to}`);
        return;
      }
  ...
  async sendSupportResponse(ticket: any, conversation: any) {
    try {
      ...
      // Bypass email sending in development if SEND_EMAILS is not set to 'true'
      if (process.env.NODE_ENV !== 'production' && process.env.SEND_EMAILS !== 'true') {
        console.log(`[EMAIL MOCK] Would send support response email to: ${this.to}`);
        return;
      }
  ...
  async sendSupportTicketClosed(ticket: any) {
    try {
      ...
      // Bypass email sending in development if SEND_EMAILS is not set to 'true'
      if (process.env.NODE_ENV !== 'production' && process.env.SEND_EMAILS !== 'true') {
        console.log(`[EMAIL MOCK] Would send support ticket closed email to: ${this.to}`);
        return;
      }
  ...
  async sendSupportAssignment(ticket: any, assignedBy?: string) {
    try {
      ...
      // Bypass email sending in development if SEND_EMAILS is not set to 'true'
      if (process.env.NODE_ENV !== 'production' && process.env.SEND_EMAILS !== 'true') {
        console.log(`[EMAIL MOCK] Would send support assignment email to: ${this.to}`);
        return;
      }
  ...
  async sendPaymentLink(subject: string, paymentLinkDetails: any) {
    // Bypass email sending in development if SEND_EMAILS is not set to 'true'
    if (process.env.NODE_ENV !== 'production' && process.env.SEND_EMAILS !== 'true') {
      console.log(`[EMAIL MOCK] Would send payment link email to: ${this.to}`);
      return;
    }
