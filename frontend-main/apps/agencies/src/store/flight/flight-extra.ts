...
export const getFlights = createAsyncThunk(
  ...
  async (_, { dispatch }) => {
    try {
      ...
      const { data } = await API.get(`/flight/getall`);
...
export const createFlight = createAsyncThunk(
  ...
  async (payload: z.infer<typeof FlightFormSchema>, { dispatch }) => {
    try {
      ...
      const { status, data } = await API.post(`/flight/create`, payload);
...
export const updateFlight = createAsyncThunk(
  ...
  async (payload: z.infer<typeof FlightFormSchema>, { dispatch }) => {
    try {
      ...
      const { status } = await API.patch('/flight/update', payload, {
        params: {
          id: payload.id,
        },
      });
...
export const cancelFlight = (id: string) => {
  return async (dispatch: Dispatch) => {
    try {
      await API.patch(`/flight/cancel?id=${id}`);
...
export const searchDirectFlights = createAsyncThunk(
  ...
  async (
    ...
    try {
      ...
      const { data } = await API.get(`/flight/search-flights`, {
        params: {
          ...rest,
