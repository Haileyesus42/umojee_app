...
import { AGENCY_ID, AGENCY_NAME, HTTP_RESPONSE, TOKEN, USER } from '../../constants/general';
import { userCridentialsProps } from '../../types/types';
import {
  updateUser,
  updateUserFields,
...
export const changeUserPassword = (userData: any) => {
  return async (dispatch: Dispatch) => {
    try {
...
export const authenticate = createAsyncThunk(
  ...
  async (credentials: userCridentialsProps, { dispatch }) => {
    try {
      ...
      if (status === HTTP_RESPONSE.SUCCESS && data.token) {
        ...
        localStorage.setItem(TOKEN, data.token);
        localStorage.setItem(USER, JSON.stringify(data.user));
        localStorage.setItem(AGENCY_ID, data.user.agency);
        localStorage.setItem(AGENCY_NAME, data.user.name);
