...
export const createTemplate = createAsyncThunk(
  ...
  async (data: any, { dispatch }) => {
    try {
      await API.post(`/announcement/templates/create`, data);
...
export const getTemplates = createAsyncThunk(
  ...
  async (_, { dispatch }) => {
    try {
      ...
      const { data } = await API.get('/announcement/templates/getall');
...
export const updateTemplate = (data: any) => {
  return async (dispatch: Dispatch) => {
    try {
      await API.patch(
        `/announcement/templates/update?id=${data.id}`,
        data,
      );
...
export const deleteTemplate = (id: string) => {
  return async (dispatch: Dispatch) => {
    try {
      await API.delete(`/announcement/templates/delete?id=${id}`);
...
export const deleteTemplates = (ids: string[]) => {
  return async (dispatch: Dispatch) => {
    try {
      await API.delete(`/announcement/templates/deleteMany`, {
        data: { ids },
      });
      dispatch(getTemplates() as any);
