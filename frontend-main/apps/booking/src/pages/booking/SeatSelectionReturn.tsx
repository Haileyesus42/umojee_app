...
const SeatSelectionPage2 = () => {
    ...
    // const serverURL = (import.meta as any).env.REACT_APP_BACKEND_URL
    ...
    const fetchSeats = async () => {
        ...
        try {
            ...
            const returnFlightId = selectedReturnFlight._id;

            const response = await axios.get(
                `${serverURL}/api/admin/seats/getallsf/${returnFlightId}`
                // `http://localhost:4001/api/admin/seats/getallsf/${returnFlightId}`
            );
    ...
    const passengerCount = selectedFlight?.totalPassengers || null;
    ...
    storeLocallyWithExpiry("selectedSeatsReturn", selectedSeats)

    const handleSaveSelection = () => {
        if (selectedSeats.length > 0) {
            // localStorage.setItem("selectedSeat", JSON.stringify(selectedSeats));
            navigate("/passengers/extra-options");
        } else toast.error("Please select your seat first!");
    };
            ...
            <div className="relative max-w-screen-2xl h-full mx-auto">
                <div className="sticky top-[70px] flex flex-col justify-center z-[1000] mx-auto">
                    {searchFlightData?.tripType === "one-way" ? (
                        ...
                        <CustomRoundtripStepper activeIndex={5} />
                    )}

                    {/* Seat status and procced nav button */}
                        ...
                        <div className="mt-4 flex mx-auto space-x-2 w-full">
                            <div className="flex justify-between w-full gap-2">
                                <button
                                    ...
                                    onClick={() => !isFetchingSeat && handleSaveSelection()}
                                >
                                    {isFetchingSeat ? "Fetching All Seats..." : "Procceed"}
                                </button>
                                {/* <button
                className="bg-red-400 text-white py-3 px-6 rounded-md hover:bg-red-500 active:scale-95 transition-all duration-500"
              ...
              >
                Clear Selection
                            ...
                            <button
                                ...
                                onClick={() => navigate("/passengers/extra-options")}
                            >
                                Skip
                            </button>
                        </div>
                                        ...
                                        {columnSeat.seats.map((rowSeat, j) =>
                                            rowSeat.hideSeat ? (
                                                ...
                                                <Seat
                                                    ...
                                                    //   passengerCount &&
                                                    //   toggleSeatSelection(rowSeat._id)
                                                    // }
                                                    // isSelected={selectedSeats.includes(rowSeat._id)}
                                                    onClick={() =>
                                                        rowSeat.status === "available" &&
                                                        passengerCount &&
...
export default SeatSelectionPage2;
