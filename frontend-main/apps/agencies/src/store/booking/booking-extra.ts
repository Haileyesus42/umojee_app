...
export const getBookedFlights = createAsyncThunk(
  ...
  async (_, { dispatch }) => {
    try {
      ...
      const { data } = await API.get(`/booking/getall`);
...
export const deleteBookingData = (id: string) => {
  return async (dispatch: Dispatch) => {
    try {
      await API.delete(`/booking/delete?id=${id}`);
...
export const deleteBookings = (ids: string[]) => {
  return async (dispatch: Dispatch) => {
    try {
      await API.delete(`/booking/deleteMany`, {
        data: { ids },
      });
      dispatch(getBookedFlights() as any);
...
export const updatePassengers = (data: {
  ...
  return async (dispatch: Dispatch) => {
    try {
      const { status } = await API.patch(`/booking/update/passenger/${data.id}`, data);
      if (status === HTTP_RESPONSE.SUCCESS) {
...
export const updateLuggages = (data: {
  ...
  return async (dispatch: Dispatch) => {
    try {
      const response = await API.patch(`/booking/update/luggage/${data.id}`, data);
...
export const bookingCheckIn = (data: {
  ...
  return async (dispatch: Dispatch) => {
    try {
      const response = await API.patch(`/booking/update/checkin`, data);
