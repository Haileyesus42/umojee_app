...
export const getAllNotificationsn = async (req: Request, res: Response) => {
    try {
        let query = ClientNotificationModel.find();
...
export const getAllNotifications = async (req: RequestWithUser, res: Response) => {
    try {
        ...
        let query = ClientNotificationModel.find({ "notifiedTo.user": userId })
            .select("_id title message route type journeyId imageUrl metadata actor notifiedTo createdAt");
        ...
        const formattedNotifications: NotificationP[] = notifications.map((notification: any) => {
            ...
            return {
                ...
                actor: notification.actor
                    ? {
                        userId: notification.actor.user?.toString() || '',
...
export const deleteAllNotifications = async (req: Request, res: Response) => {
    try {
        const result = await ClientNotificationModel.deleteMany({})
...
export const updateNotificationSeen = async (req: RequestWithUser, res: Response) => {
    try {
        ...
        console.log("Client Notification ID:", notificationId);

        // Find and update the `seen` field in the correct `notifiedTo` entry
        const updatedNotification = await ClientNotificationModel.findOneAndUpdate(
            {
        ...
        if (!updatedNotification) {
            console.error(
                "Client Notification not found or not associated with the user:",
                { notificationId, userId }
            );
            return res
                .status(404)
                .json({ status: "fail", message: "Client Notification not found or not owned by user" });
        }

        res.status(200).json({
            status: "success",
            message: "ClientNotification marked as seen",
            data: updatedNotification,
        });
        console.log(res.statusCode, "Update Success!!!");
 ...
 */
export const updateAllNotificationsSeen = async (req: RequestWithUser, res: Response) => {
  try {
    const userId = req.userId;

    // Update all notifications for this user where the notification is not seen
    const updateResult = await ClientNotificationModel.updateMany(
      {
        "notifiedTo.user": userId,
        "notifiedTo.seen": false,
      },
      {
        $set: { "notifiedTo.$[elem].seen": true },
      },
      {
