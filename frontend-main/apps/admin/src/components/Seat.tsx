...
const Seat = ({ seatData, onClick, isSelected }: SeatProps) => {
  ...
  return (
    <div
      className={`group ${status !== "unavailable" && "active:scale-[0.98]"
    ...
    >
      ...
      >
        <div
          className={`absolute ${armTrayLeft ? "bg-gray-800" : "bg-slate-100"
            } left-0 bottom-0 h-[80%] border border-gray-600 rounded-t-md w-2 -translate-x-1/2 `}
        />
        <div
          className={`absolute ${armTrayRight ? "bg-gray-800" : "bg-slate-100"
            } right-0 bottom-0 h-[80%] border border-gray-600 rounded-t-md w-2 -mr-[1.5px] translate-x-1/2 `}
        />
        {handicapArmRest ? (
          <HandicapIcon
            className={`text-2xl ${isSelected ? "text-white" : "text-blue-600"
              } -mb-1`}
          />
        ) : (
          unsuitableForHandicap && (
            <NoHandicapIcon
              className={`text-[21px] -mb-1 ${isSelected ? "text-white" : "text-red-600"
                } mt-1`}
            />
          )
        )}
        <span
          className={`font-semibold drop-shadow-lg ${isSelected ? "text-white" : "text-black"
            }`}
        >
          {seatId}
        </span>
